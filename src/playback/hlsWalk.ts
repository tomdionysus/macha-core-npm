...
/**
 * Stop a response body without reading it.
 *
 * A node that ignores `Range` answers `200` with the whole file, and for a
 * progressive source that is the film. Nothing here reads the body; this makes
 * sure the host does not go on transferring it into nothing either.
 */
function discardBody(response: Response): void {
  const body = (response as unknown as { body?: StreamingBody | null }).body;
  try {
    body?.getReader?.().cancel();
  } catch {
    // Already finished, or a host with no streaming body: nothing to stop.
  }
}

/**
 * Is this source still being served? Answers for **any** source, and reads no
 * payload.
 *
 * The question a host must answer before reporting a terminal failure, because
 * what core does next depends entirely on the kind: a `not-found` is checked
 * against the session route and regenerated on the same node with nothing
 * charged, while an unclassified failure is evidence against the node and
 * walks away from it.
 *
 * **Progressive sources had no way to ask**, and that is how two reaps on the
 * Android TV set on 2026-09-23 each failed over from a healthy local node to
 * one across the internet. Its player surfaces a direct-play fetch error as a
 * bare message with no status, and the adapter, with no probe it could run
 * without range-requesting the film, reported it as `unknown`. So this ranges
 * the source itself for **one byte**, `bytes=0-0`, and cancels whatever body
 * comes back in case the node ignored the range. The node answers a deleted
 * session's direct URL `404` on the same path that serves it, so the answer
 * maps through `playbackFailureKindForStatus` like any other.
 *
 * A manifest is walked by `probeHlsReadiness`, unchanged, so a host can call
 * this for every source without asking which kind it has.
 */
export async function probeSourceReadiness(
  source: PlaybackSource,
  options: HlsWalkOptions,
): Promise<HlsWalkOutcome> {
  if (source.isManifest) return probeHlsReadiness(source, options);
  let response: Response;
  try {
    response = await walkFetch(source.url, source, HLS_READINESS_RANGE, options);
  } catch (error) {
    return { state: 'unavailable', detail: causeDetail(error) };
  }
  discardBody(response);
  return response.ok ? { state: 'ready' } : { state: 'unavailable', status: response.status };
}
