...
- **The signature does not churn.** Two catalogue fetches one second apart: **418 refs, 418 identical URLs**; re-measured 2.6 hours later, **836 refs, one distinct `exp`, byte-identical on two independent nodes, not moved by a millisecond.** `exp` is midnight-aligned.
  ...
  **The lesson, and it cost two sessions an hour: measure `exp` against the deployed cluster, never against the server's source.** Both readings were correct about the build in front of them, and neither was correct about the other's. Ask what version answered before reconciling anything.
