...
- **The artwork host is chosen by this viewer's round trip, once per run** (`154270d`), for Tom's artwork P0. It moves only on a gain of at least 50 ms and 40%, and it stays sticky. Measured before: 636 ms per cold poster from macnessa against 65 ms from fi-1 on the LAN. Not yet re-measured after. The server's share (cold reads, daily-rotating URLs, oversized posters) is the server's.
