...
### ~~`segment_not_ready` as a `500` is under review~~ — hls.js answered, Android TV outstanding

**Raised 2026-09-20 by Tom through the server session**, with core coordinating one answer rather than three. **hls.js is in and the constraint holds. The Android TV stack has been asked and has not answered.**
...
**Conclusion, pending the Android TV answer: `500` stays, now with evidence and a date.** Written into `docs/writing-a-player.md` so nobody re-derives it, with `425` recorded as the option that becomes available if a coordinated release is ever worth it. The web client reached the same conclusion independently.
...
**Not taken: an empirical 425 fixture.** The web client offered to stand one up and watch hls.js against it. Declined for now — the source reading plus the `shouldRetry` finding settles the mechanics, and the decision now turns on the per-client opt-in cost, which no fixture measures.
