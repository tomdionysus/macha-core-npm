...
- **`moveTo` now has a door, and no caller yet.** It had none because a host holds a `PlaybackRuntime`, never the coordinator, and the runtime did not forward it. `PlaybackRuntime.moveTo(endpointId)` landed on `develop` in `2f196a1` on 2026-09-23, **unreleased**: the coordinator's answer with a live generation, `false` when idle or failed (a failed generation is already released, so a host spells that as `prefer()` then `retry()`). Asked for by the web client, which is wiring `selectNode` to it through its link; **nothing has moved a live session between nodes yet.**
