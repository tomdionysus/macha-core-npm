...
**Implementation note, and it is the part core could not have seen.** The Android TV client flags `MachaProvider.tsx:113-115`, which calls `stop()` in an effect cleanup, against `:137` calling `start()` again on `AppState` `active`. **So on React Native there is a routine window, every background/foreground cycle, in which an in-flight request is issued after `stop()`.** Today it yields a 401; under the new behaviour it throws "not started".

That is a lifecycle transition, not a programming error, and the two should probably not read the same. **Distinguish them:** never started is a caller mistake and should be loud; stopped-and-about-to-restart is normal on a mobile platform and happens in milliseconds. At minimum the thrown error must let a client tell which it is, or every backgrounding turns into a visible failure. **A browser tab does not background this way, so the web client will not show it** — the same shape as the storage-cache coupling, where the platform that cannot exercise the bug is the one that reports green.
