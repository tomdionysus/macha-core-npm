...
**`develop` is now well ahead of `0.14.0` and none of it is published.** Twenty-four commits on 2026-09-20. The first seventeen are core-internal: the runway re-read at the decision point, advisory status routing, `close()` awaiting the recovery it does not own yet, the session-manager generation, the health loop surviving a full store, the success-envelope validation, `find`'s absence reporting, the fake-player merge, the already-charged close, six from the low register, and the recovery transform restatement. The last seven came out of one evening of cross-session work and **three of them are viewer-facing**: the playlist status kept rather than discarded (`10a1d93`), the unclassified-failure diagnostic (`c5ee31d`), the encoder-speed reading half, and the **regeneration hang fix** (`0e787f8`). **962 tests, typecheck, `lint:platform`, `build` and `dist:check` all clean — in that order, build last.** Everything marked *BUILT on `develop`, unreleased* in this file is in that set.

**`dist` is no longer only core's business.** The phone client is on a `file:` link to this tree's `develop` as of 2026-09-20, so **a change here reaches another repository as soon as `dist` is rebuilt, with no publish in between**. Build before ending a session that touched `src`. A half-built `dist` is a client reading a directory core is mid-write on, which is what three of the four historical link failures actually were.

**A release is owed and the reason is no longer "several nice fixes".** `0e787f8` fixes a viewer freezing indefinitely, on a path `10a1d93` opens. **Those two must ship together or not at all** — shipping the classification fix alone moves a live reap off a path that recovers badly onto one that hangs. That is not a theoretical ordering: it is what happened on the television tonight, in that order, within the hour.
