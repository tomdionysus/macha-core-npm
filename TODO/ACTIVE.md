...
### Core estimates what a generation start costs, and a move declines a join it cannot win
**Waiting on:** Tom to confirm the ruling first-hand and say when, then core. **Ruled 2026-09-23 and relayed by the server session, not heard here directly:** no node will state a time-to-first-fragment; measuring or estimating it, and deciding whether to join, is core's job.
