...
### The law numbers mean different things in core and on the server — and both trees cite by number

**Raised 2026-09-20**, when the server session sent core the law set and it did not match core's copy. **Nobody should renumber either side without Tom**, because every existing citation in both trees moves with it.

**The conflict.** `docs/principles-and-laws.md` has **control at 1, viewer at 2, loader at 3**. The server's `ARCHITECTURE.md`, as of today, has **viewer at 1, loader at 2, control at 3**, and the server says its laws are *"cited by number in the server source"*. So "Law 1" means control here and viewer there — and this file cites the numbering in `docs/`, for instance in *P1 — correctness* and in the `pipeline_idle_ms` entry. Anyone reading across the two trees reads those backwards.

**The content of the three is the same; the order and one framing differ.** Core states a strict class order, `control > viewer >> loader > speculative`. The server states that its law 3 is *not* subordinate to its law 1 — *"a floor law 1 may not eat through, which is the whole force of 'a large share, not indefinite starvation'"*, with a configurable 95:5 default. Those are close to the same claim from opposite ends and may well be one law written twice, but they are not identical and this is not core's to flatten. **See the divergence rule: two copies disagreeing is a case to compare, not a merge to perform.**

**Law 4 is not in conflict and has been adopted here unchanged**, because neither tree previously had a fourth: *Thou Shalt Not Shoot Thyself In The Foot* — no operation may leave the node, or the client, in a state it cannot recover from on its own. It is a veto over the priority laws rather than a rank among them, and its test is *"if this goes wrong on the node furthest away, does it come back without me?"* The client's version is a television in another room nobody will relaunch. It arrives with five self-healing disciplines, four of which bite in core.

**Three of those disciplines already have items in this file, and naming them is worth more than the items were separately:**
- ***A bound smaller than one unit of its own work is not a bound.*** This is the cooldown-ladder entry in the low register verbatim — 500 ms and 2 s against a 10 s probe interval — and it is also why `replacementLeadTimeMs` has an attempt-budget floor. The register entry says the constants do not record their relation; the discipline says why that is a defect and not a tidiness complaint.
- ***Re-derive, do not assert.*** The runway re-read at the decision point is exactly this, built the day before the law existed.
- ***Every retried work item gets backoff, a failure budget, a parked state and an operator action.*** `releaseFailedSession`'s close ladder satisfies all four; the facts retry has a budget; **the cooldown ladder has backoff and no budget, no parked state and no operator action**, which is a new way of reading an entry that has sat in the register as cosmetic.

**What is actually being asked:** whether core renumbers to match the server, the server renumbers to match core, or the two stay divergent with a note. Cheap to do, expensive to do twice, and not core's call.
...
**One thing still outstanding, and it is hardware.** The Android TV client will re-run its §1.0 reap on *Life of Brian* once the television is powered on, capturing the requested `mode` on the create beside the echoed transforms. Its `playbackOptions.ts` already documents a 5.1 downmix as arriving `mode: 'transcode', video: 'copy', audio: 'transcode'`, which makes "the generation was a transcode with the video untouched and the mode label agreed with it" far more likely than a node substitution — the branch this fix is built for. The capture is confirmation, not a fork.
...
### `bad_playback_request` cannot tell### `bad_playback_request` cannot tell "this node cannot" from "your request is wrong", and core steps down on both

**Waiting on:** core for the mitigation, the server for the real fix. **Found 2026-09-20**, by the server session answering core's question about the 400 rather than by core reading anything. `PlaybackCoordinator.ts` — `isExecutorRefusal`, `degradedInstructionFor`, `recoverWithPreferences`.

**What core does.** A **400** from a generation attempt is read as *"this node cannot perform this instruction"* — as distinct from 429 capacity and 5xx health — and the instruction is degraded once: the copy is given up and the node is asked again for a transcode.

**What the server actually returns.** A copy into a container that cannot carry the codec throws `std::invalid_argument` (`src/playback.cpp:428-440`), caught at `:2962` and returned as 400 with code **`bad_playback_request`**. So far so good. But that is the code for *every* `invalid_argument` on that path — an unknown mode string, a non-string `video`, a bad container value, and "this node cannot copy that codec" all return the identical code, distinguished only by a human-readable message. **Core's classifier cannot see the difference, so a malformed request from core gets silently downgraded and retried instead of reported.** The server's own words: *"if you step down on every `bad_playback_request` you will also step down on your own bugs and mask them."*

**Why this is smaller than it sounds, and exactly how much smaller.** Core cannot easily *send* the malformed shapes: `mode`, `video`, `audio` and `container` are all union types, so an unknown mode string or a non-string `video` cannot survive `tsc`. And the step down on the recovery path is already narrowed to requests that **asked a node to copy something** — a 400 for any other reason is rethrown untouched. What remains is a genuine hole rather than a theoretical one: a host-supplied preference reaching `instructedPreferences` untyped at runtime, or a pair core assembles that the server rejects as incoherent, would be masked as a capability refusal and retried once.

**Three things to do, and only the first is core's alone.**
1. **Say what is not known.** `instruction-degraded` currently reads as though core established that the node could not copy. It should record that the 400 was undiscriminated, so a masked bug is at least visible in the trail — and on the Android TV client warn-level events are now on screen for the whole of a film, so this is a surface a person sees.
2. **Do not key on the message.** It is prose, it is not contract, and matching it would make core's recovery depend on server wording.
3. **Ask for a distinct code.** The server session has identified this as a real server-side item and has *not* raised it, because it does not change server code on a client session's conclusion without its operator. **Core should ask for it to be raised rather than work around it** — a `code` distinguishing a capability refusal from a bad request is the fix, and everything above is mitigation until it exists.
