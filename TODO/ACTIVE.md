...
**`npm run build` cleans `dist` first, and must keep doing so.** `tsc` does not remove output whose source is gone, so deleting a file used to leave an orphaned `dist/.../x.js` and `.d.ts` behind for every client to resolve. `dist:check` **cannot catch this** — it compares mtimes of files that exist, so a deleted source is precisely the case it is blind to. Found by the web client after `VolumeStore` was removed and `dist/state/volume.js` was still sitting in its `node_modules`, which is exactly the thing that makes someone believe a deletion did not happen.
