...
- **Can the node sustain the stream from here?** That is recorded media throughput against the bitrate. It is meaningful since web client `98f4e3c` feeds hls.js fragment bytes to `recordTransferByUrl`, where before it was JSON. **A decline on this is worth building and is not built.** Only judge it against enough media samples; note that `EndpointBandwidth.restore()` re-enters at one sample against a threshold of two.
