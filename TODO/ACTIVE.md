...
### `segment_not_ready` as a `500` is under review, and core's recorded reason is not the one being defended

**Waiting on:** both client sessions for evidence, then Tom. **Raised 2026-09-20 by Tom through the server session**, with core asked to coordinate one answer rather than three. **Core is the integration point here by his instruction.**

**The challenge.** 5xx means the server got it wrong, and a fragment not produced yet is not that — the node is working exactly as designed and the request is valid, merely early. That reads as `404`, `425 Too Early` or `409`. The reason it is `500` is believed to be that **a 4xx stops the player retrying at all**, and that the status is the only readable discriminator on a fragment error. Tom's question is whether that is *still* true, since it was decided against particular player versions.

**What core can answer now — Q4, what would break — and the server's guess about it was wrong.** Fragments never reach `retryableEndpointFailure`; that is the API-request classifier. A fragment status goes through `playbackFailureKindForStatus`, so the cluster walk is not involved. **The hazard is the fallback.** That function maps `500`/`404`/`503` and sends everything else to `unknown`, which is deliberately treated as possible endpoint evidence. So a node moving to `425` against a client on an older core sends the hold to `unknown`: **a node producing correctly is charged, a standby is built elsewhere, and failover can escalate** — the exact failure the `not-ready` kind exists to prevent, reintroduced by version skew. **Any change here is a coordinated release, core first.**

**The cheap mitigation, available whenever there is a decision to build toward.** Two lines: accept the new status *and* `500` as `not-ready`, released before any node moves, so the change is safe in either order and the server is not gated on client rollout. The same release should map the `410` the server is adding for a superseded generation, which core would otherwise send to `unknown` and treat as endpoint evidence — wrong for a generation the client itself superseded.

**Fixed while checking, and latent regardless of the outcome:** `hlsWalk.ts` spelled `500` inline at two sites instead of using the `SEGMENT_NOT_READY_STATUS` constant it already exports. A hold that stopped matching would have fallen to `!response.ok` and condemned a working node. Now one definition in core rather than three.

**The finding that may change the shape of the argument.** Core's documented reason for the `500` is **not** the reason being defended. `docs/writing-a-player.md` argues `500` over **`503`**, and the argument is about intermediaries: every proxy and load balancer emits `503` for a service genuinely down, so give the hold `503` and a dead node behind one reads as healthy — no failover ever, nothing surfaced. **That rules out `503` and says nothing about `404` or `425`.** The separate objection to `404` is also about caching, not retries: the playlist has promised the object exists, so a `404` invites an intermediary to cache the absence.

**So both recorded arguments are about intermediaries, and neither excludes `425`** — not cacheable by default, not a status a proxy emits on its own. "A 4xx stops the retry" survives in core's docs only as a claim about what each *player* does, **attributed to no measurement and no date**. That is exactly the rot Tom is asking about. If hls.js and media3 retry a `425` sensibly there may be no recorded objection to it at all.

**Core is not arguing for a change** — it is reporting that the constraint is narrower than either document claims and that the evidence to settle it is not in this tree.

**Asked of the clients**: does a 4xx on a fragment still stop the retry; is the body or any header readable on a fragment error yet; is `425` viable anywhere; and for Android TV, whether the body limit is expo-video's or media3's underneath — because a wrapper gap someone could close is not the same fact as a permanent one. **"Still forced, nothing has changed" is a perfectly good outcome** and gets written into the document with the evidence and the date.

**Core's own Q2 answer, pending confirmation:** hls.js reports a bad status as `{ code, text }` with `response.data` undefined, so an error envelope never reaches the error event — reachable only via `data.networkDetails`, the raw `XMLHttpRequest`, which becomes something else under `FetchLoader`. The status is the one field both loaders populate identically, which is the whole reason it is the discriminator.
