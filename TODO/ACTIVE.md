...
**`develop` is now well ahead of `0.14.0` and none of it is published.** Twelve commits on 2026-09-20, all core-internal: the runway re-read at the decision point, advisory status routing, `close()` awaiting the recovery it does not own yet, the session-manager generation, the health loop surviving a full store, the success-envelope validation, `find`'s absence reporting, the fake-player merge, and six from the low register. **921 tests, typecheck and `lint:platform` clean.** Every item marked *BUILT on `develop`, unreleased* in this file is in that set, and **no client can see any of it** — they resolve from the registry. Two things a release has to carry into its note rather than let a client discover: `FakePlayer.detach()` now stops (it ships on the `./testing` export), and `PlaybackEvent.forwardBufferMs` gained a contract both RN clients have already been told about and checked themselves against.
