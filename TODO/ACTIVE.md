...
All three re-verified on request rather than answering from memory, and each confirmed no removed or renamed symbol resolves in its tree. The web client's number came from `npm test`, so `dist:check` ran first — the suite was green against a **current** build rather than a stale one, which given the day is the part of "tested" that was actually in doubt.

**Two clients have a gate that already reads the degraded state**, which is why the re-mint case above is load-bearing rather than theoretical. The Android TV client maps `sessionLockedOut(roles)` straight to a sign-in wall — but a latch admits the client once and never un-admits it, deliberately, so a mid-session failure cannot tear down the player and throw a wall in front of someone mid-film. The honest shape there: **the viewer is not interrupted, their library requests simply start failing with nothing on screen explaining why, and they meet the wall at next launch.** Exactly the auth event wearing the costume of a UI bug that `lastIdentityChange` exists to name.
...
**`npm run build` compiles to `dist.staging` and renames it into place. Keep it that way.** Every client resolves this package through a `file:` link, so their suites read `dist` *live*, with no version boundary to hide behind — and this failed in both available directions in one afternoon:

- Emitting straight into `dist` left **orphans**: `tsc` removes nothing, so a deleted source keeps its output. `dist/state/volume.js` was still resolvable in a client's `node_modules` after `VolumeStore` was deleted, which is exactly the artefact that makes someone believe a deletion did not happen. **`dist:check` cannot catch it** — it compares mtimes of files that *exist*, so a vanished source is the one case it is structurally blind to.
- Emptying `dist` first made it **worse**. Before, a client building mid-rebuild got something stale but complete: importable, suite runs, probably passes. After, it got a directory that was absent or partial and every import failed at once — one client watched its suite collapse to "45 files failed / no tests" and went looking for a breakage in its own tree first. *A loud failure in somebody else's repo costs them the search.*

Staging plus an atomic `rename` closes both instead of trading one for the other: staging starts empty so orphans cannot survive, and a reader sees the old tree or the new one and never a tree mid-write. A failed compile leaves `dist` untouched, verified — a broken build must not be able to break four clients with it.
...
2. **Volume and mute — REVERSED on the same day. Core ships nothing.** Tom first said "explicit mute please"; shown the proposed model, he reversed it: *"Don't second guess the client. If the user muted, or starts with zero volume, that's what you do. That's not core. In fact, why is this in core at all? It's player logic."* **So there is no mute concept in core and `state/volume.ts` is unchanged.** The Android TV client's `{effective, setting, muted}` model was right and stays in its tree.
   ...
   **What followed, and it goes further than the mute question.** The web client audited everything volume-shaped in core and found four things of unequal merit:
   ...
   - **`PlaybackRuntime.setVolume` — STAYS. This file was wrong about it, twice, and told two clients so.** It was described here as "a one-line passthrough that exists only to carry the above". It is not: it touches `VolumeStore` **nowhere**. It is `this.player.setVolume?.(volume)`, a delegation to `Player.setVolume?` — which core is *keeping*. The store **persists** a volume; the runtime method **applies** one to the active player. They share a word, not a concern, and moving one implies nothing about the other. *Caught by the web client when it was told to "call your player adapter directly instead" — which is not actionable, because `PlaybackRuntime.player` is private and constructed internally by `platform.createPlayer()`, so no client ever receives that reference. The only way to honour the advice would be to expose the player, which is a far worse change than a one-line delegation.*
...
- **The signature does not churn.** Two catalogue fetches one second apart: **418 refs, 418 identical URLs**; re-measured 2.6 hours later, **836 refs, one distinct `exp`, byte-identical on two independent nodes, not moved by a millisecond.** `exp` is midnight-aligned.

  *This looked like a flat contradiction of the server's source reading (`exp = unix_ms() + ttl`, millisecond granularity, no bucket) and was raised as one. **It was not a contradiction: they were two different builds.** The server's bucketing shipped in `0.40.0` and was deployed to both nodes that afternoon (16:33 and ~17:45); the measurement was taken after. `exp = (now/ttl + 2) * ttl` with a 24 h TTL is necessarily a multiple of 86,400,000 ms from the Unix epoch, and the epoch is itself a UTC midnight — so midnight alignment and one value for every object in a response are exactly what that change produces. The `0.38.x` attribution in the first report was the client's own sourcing error, corrected by it from `/api/v1/status`.*

  **The lesson, and it cost two sessions an hour: establish which build answered before reconciling anything.** It cuts both ways — the server read source and reported it as live behaviour; a client measured live and reported it as source. Both readings were correct about the build in front of them and neither about the other's. `/api/v1/status` carries the version; ask it first.

  **The invariant, confirmed from source by the server session and now recorded on `expiredCapability`:** the expiry is `(now / ttl + 2) * ttl` — floor to the current bucket, then add **two**, not one. So remaining validity is always in **(24 h, 48 h]**, and the floor is *strictly* greater than the `max-age=86400` the artwork response carries. **A cached copy can never outlive the signature that names it, at any point in the cycle.** The "plus two" is the whole point: a naive next-boundary bucket would hand a URL minted at 23:58 a two-minute signature behind a 24-hour cache directive, invisibly to the client holding it. *The web client inferred the rule from two samples and explicitly declined to claim it; the server confirmed it deliberately. That is the right shape for both halves.*

  **What it means for core:** `expiredCapability` can no longer fire on a freshly-read payload. Its one remaining case is **a payload held across a bucket boundary** — persisted state, a long-lived cache, a client resuming after a long idle — which is exactly the artwork residue the Android TV client reported. Recorded on the function so a future hit is not read as a server fault.
