...
**`develop` carries a prerelease version, and the rule is that it must never equal a tag.** *Found 2026-09-21 by the phone client, and it is the day's fault class one more time.* `0.17.0` was tagged at `c3d840b`; `develop` then moved to `9a37ce3` with `playbackFailureStatus` and the deferred release — **250 lines of source apart, both answering `"version": "0.17.0"`.** Four clients are hotlinked and any of them grepping a version got a number that agreed with a tag it did not contain. **A version that agrees while the contents differ is exactly how this fleet lost an afternoon to a symlink and how a client shipped `versionCode 1` four times.**

**So `develop` is `0.18.0-dev` and stays on a `-dev` prerelease between releases.** A client reading it learns *"this is a development tree"* rather than a false release number, which is the whole job of the string. The release commit bumps it to the bare semver and the tag goes on that, as before — **the version bump stays inside the release commit**, so the tag still points at exactly what ships.

**Not done, and deliberately: tagging `0.17.1` at the current tree.** The phone client offered it and it is cheap, but it names one commit and `develop` moves again immediately, so it buys a correct number for about an hour. **The identifier for a linked tree is the SHA plus a hash of `dist`, not a version string** — that client already records both, and its detail is worth keeping: hash from *inside* `dist`, because `shasum` includes the path it is given and two runs from different directories disagree about identical bytes.

**What core must not do, which that client named before core could get it wrong:** bump `package.json` without tagging, because then the tree and the tag disagree in the other direction and the next client to grep gets a differently wrong answer. The `-dev` suffix avoids that by not claiming to be a release at all.
