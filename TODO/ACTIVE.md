...
**The top of *P1 — correctness* is now a `Waiting on Tom` question in disguise, and it is the release gate.** The transform restatement is built and tested, but it rests on the server honouring `video`/`audio` at session *creation* and refusing an impossible copy with a **400**. Both were asked of the server session on 2026-09-20 and neither has come back. If the creation half is wrong the fix is inert; if the refusal is a 5xx rather than a 400 the single step down never fires and a replacement node that cannot copy takes the viewer terminal. **Do not tag a release carrying this until both are answered.** Two things a release has to carry into its note rather than let a client discover: `FakePlayer.detach()` now stops (it ships on the `./testing` export), and `PlaybackEvent.forwardBufferMs` gained a contract both RN clients have already been told about and checked themselves against.
...
**Two things core cannot settle from here, both asked 2026-09-20.** The Android TV client has been asked whether the replacement node in its measured case would have *accepted* `video: 'copy'` — the difference between this restoring the passthrough and it merely making the refusal visible. The server has been asked to confirm that the 0.34.0 restatement rule applies to session **creation** as well as to a PATCH, that `mode: transcode` with `video: copy` is a combination admission accepts rather than normalises away, that a node unable to copy refuses with **400** rather than a 5xx or a silent substitution, and whether there is a `reason` or `code` to key on instead of the bare status. **If the creation half is wrong, this fix does nothing and the entry reopens.**
