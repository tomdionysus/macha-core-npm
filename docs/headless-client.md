...
**5 — Playback negotiation happens with no player in sight.** `playbackResolver.resolve()` returns the server's decision: mode, stream URL, what it chose to transcode and why. That decision is the entire input to a `Player`, which is why the core can negotiate playback on a machine that cannot decode video. Advertise capabilities honestly — over-claiming is how you get a black screen, and under-claiming only costs an unnecessary transcode.
