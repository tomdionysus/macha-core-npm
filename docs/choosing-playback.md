...
**The core consumes these; a host must not — when a `PlaybackCoordinator` is doing the converting.** It converts between the title's timeline and the generation's and hands a player a generation-local position, so a host that also corrects by the offset double-corrects, and the result is self-consistent and wrong. **A host driving `ClusterPlaybackResolver` directly has no such layer**, and the rule inverts: nobody is adding the generation's origin, so it must. See [Driving the resolver directly](resolver-direct.md), and note that the missing term there is `seekMs` rather than `seekOffsetMs` — the offset is zero on transcode and direct and already inside `seekRequestedMs` on a remux. `checkSeekInvariant` verifies the sum once, for every client, and reports a violation without acting on it — a clamp cannot trip it, because the sum balances against the honoured position.
